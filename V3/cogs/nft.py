...
class nft(commands.Cog):
    ...
    async def nft(
        ...
        collection: Option(str, 'Specify the collection slug', autocomplete=collection_name_data),
        ...
        rarity_button = Button(
            label='Rarity', style=discord.ButtonStyle.blurple, emoji='💎')
        lastSale_button = Button(
            label='Last Sale', style=discord.ButtonStyle.green, emoji='💳')
        ipfs_button = Button(
            label='IPFS', style=discord.ButtonStyle.link, emoji='🧊')
        return_button = Button(label='EXIT', style=discord.ButtonStyle.red)
        gitbook_button = Button(label='GitBook', style=discord.ButtonStyle.link,
                                emoji='<:gitbook_button:1047912317427400704>')
        gitbook_button.url = 'https://kizmeow.gitbook.io/kizmeow-nft-discord-bot/information/faq'
        gitbook_button.custom_id = None
        invite_button = Button(label='Kizmeow Support Server',
                               style=discord.ButtonStyle.link, emoji='<:kizmeow:1047912736224448562>')
        invite_button.url = 'https://discord.gg/PxNF9PaSKv'
        invite_button.custom_id = None
        # ----------------------------------------------------------------------------------------------------------------------------------------------------------------
        await ctx.defer()
        load_dotenv()
        url = f'https://api.modulenft.xyz/api/v2/eth/nft/token?slug={
            collection}&tokenId={token_id}'
        headers = {
        ...
        nft_image = 'https://i.imgur.com/aSSH1jL.gif' if r['data']['metadata'] == {
        } else r['data']['metadata']['image']
        nft_image = re.sub('ipfs://', 'https://ipfs.io/ipfs/', nft_image)
        nft_owner = '0x0000000000000000000000000000000000000000' if r['data']['owner'] == {
        } else r['data']['owner']['owner']
        collection_image = 'https://i.imgur.com/aSSH1jL.gif' if r['data']['collection'][
            'images']['image_url'] == None else r['data']['collection']['images']['image_url']
        collection_contract_address = 'no data' if r['data']['collection'][
            'contractAddress'] == None else r['data']['collection']['contractAddress']
        nft_IPFS_image = 'https://i.imgur.com/aSSH1jL.gif' if r['data']['metadata'] == {
        } else r['data']['metadata']['image']
        nft_IPFS_image = re.sub(
            'ipfs://', 'https://ipfs.io/ipfs/', nft_IPFS_image)
        ...
        # ----------------------------------------------------------------------------------------------------------------------------------------------------------------
        url = f'https://api.traitsniper.com/v1/collections/{
            collection_contract_address}/nfts?token_ids={token_id}'
        headers = {
            'accept': 'application/json',
        ...
        async def initial_embed():
            ...
            embed.set_author(name=f'{collection_name}#{token_id}', url=f'https://opensea.io/assets/{
                             collection_contract_address}/{token_id}', icon_url=collection_image)
            embed.add_field(name='Owner', value=f'[{
                            nft_owner[0:6]}](https://etherscan.io/address/{nft_owner})', inline=True)
            embed.add_field(name='Type', value=erc_type, inline=True)
            embed.set_footer(text='Data provided by Kizmeow NFT Bot',
                             icon_url='https://user-images.githubusercontent.com/80938768/204983971-d7cf0e40-f4ce-4737-ba07-85ed62112dab.png')
        ...
        async def return_button_callback(interaction):
            if ctx.author == interaction.user:
                ...
                embed = discord.Embed(
                    title='Consider use `/nft` command by yourself.', color=0xFFA46E)
        ...
        async def rarity_button_callback(interaction):
            if ctx.author == interaction.user:
                if traitsniper_available:
                    embed = discord.Embed(title=f'{collection_name}#{
                                          token_id}', color=0xFFA46E)
                    embed.set_image(url=nft_image)
                    embed.add_field(name='Owner', value=f'[{
                                    nft_owner[0:6]}](https://etherscan.io/address/{nft_owner})', inline=False)
                    embed.add_field(
                        name='Rank', value=rarity_rank, inline=True)
                    embed.add_field(name='Score', value=f'{
                                    rarity_score:.2f}', inline=True)
                    embed.set_author(name=collection_name, url=f'https://opensea.io/assets/{
                                     collection_contract_address}/{token_id}', icon_url=collection_image)
                    embed.set_footer(text='Data provided by Kizmeow NFT Bot',
                                     icon_url='https://user-images.githubusercontent.com/80938768/204983971-d7cf0e40-f4ce-4737-ba07-85ed62112dab.png')
                    embed.add_field(
                        name='═════════════Traits═════════════', value='_ _', inline=False)
                    ...
                    embed = discord.Embed(
                        title=f'{collection_name} is not available on traitsniper.', color=0xFFA46E)
                    embed.set_author(name=collection_name, url=f'https://opensea.io/assets/{
                                     collection_contract_address}/{token_id}', icon_url=collection_image)
                    embed.set_footer(text='Data provided by Kizmeow NFT Bot',
                                     icon_url='https://user-images.githubusercontent.com/80938768/204983971-d7cf0e40-f4ce-4737-ba07-85ed62112dab.png')
            ...
            if ctx.author == interaction.user:
                if last_sale_exist:
                    ...
                    embed.add_field(name='From', value=f'[{
                                    from_address[0:6]}](https://etherscan.io/address/{from_address})', inline=True)
                    embed.add_field(name='To', value=f'[{
                                    to_address[0:6]}](https://etherscan.io/address/{to_address})', inline=True)
                    embed.add_field(name='It was', value=f'<t:{
                                    timestamp}:R>', inline=True)
                    embed.add_field(name='Sales Price', value=f'{
                                    sale_price} ETH', inline=True)
                    embed.set_author(name=collection_name, url=f'https://opensea.io/assets/{
                                     collection_contract_address}/{token_id}', icon_url=collection_image)
                    embed.set_footer(text='Data provided by Kizmeow NFT Bot',
                                     icon_url='https://user-images.githubusercontent.com/80938768/204983971-d7cf0e40-f4ce-4737-ba07-85ed62112dab.png')
                    ...
                    embed = discord.Embed(title=f'{collection_name}#{
                                          token_id} is never sold', color=0xFFA46E)
                    embed.set_author(name=collection_name, url=f'https://opensea.io/assets/{
                                     collection_contract_address}/{token_id}', icon_url=collection_image)
                    embed.set_footer(text='Data provided by Kizmeow NFT Bot',
                                     icon_url='https://user-images.githubusercontent.com/80938768/204983971-d7cf0e40-f4ce-4737-ba07-85ed62112dab.png')
